...
protected:
    ...
    virtual void swap( size_type /*i1*/, size_type /*i2*/ ) {}

    /// Reorder the values.
    virtual void renumber( const sofa::helper::vector<size_type> &/*index*/ ) {}

    /// Add some values. Values are added at the end of the vector.
    /// This is the (old) version, to be deprecated in favor of the next method
    virtual void add( size_type /*nbElements*/,
    ...
    virtual void add( const sofa::helper::vector<size_type> & index,
            const sofa::helper::vector< TopologyElementType >& elems,
            const sofa::helper::vector< sofa::helper::vector< size_type > > &ancestors,
            const sofa::helper::vector< sofa::helper::vector< double > >& coefs,
            ...
            )
    {
        ...
        add((size_type)index.size(), elems, ancestors, coefs);
    }

    /// Remove the values corresponding to the ELement removed.
    virtual void remove( const sofa::helper::vector<size_type> & /*index */) {}

    /// Move a list of points
    virtual void move( const sofa::helper::vector<size_type> & /*indexList*/,
            const sofa::helper::vector< sofa::helper::vector< size_type > >& /*ancestors*/,
            const sofa::helper::vector< sofa::helper::vector< double > >& /*coefs*/) {}

    /// Add Element after a displacement of vertices, ie. add element based on previous position topology revision.
    virtual void addOnMovedPosition(const sofa::helper::vector<size_type> &/*indexList*/,
            const sofa::helper::vector< TopologyElementType > & /*elems*/) {}

    /// Remove Element after a displacement of vertices, ie. add element based on previous position topology revision.
    virtual void removeOnMovedPosition(const sofa::helper::vector<size_type> &/*indices*/) {}
